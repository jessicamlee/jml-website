<header id="header" data-aos="fade-down">
    <div class="container header-bar flex">
        <a class="logo flex" href="https://jessicamlee.dev/" title="Home Page">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 960.32 936" alt="Jessica Lee Logo"><defs></defs>
                <path data-aos="fade-down" data-aos-delay="600"d="M960.32,446l-63.41,50.27c-45-.47-101.54-1.13-156.86-1.77-77.9-.91-151.48-1.76-183-2l-25.25-.16V467.16c0-38.22-.72-297.82-1.77-466.84L580.92,0c.91,147.29,1.56,363.36,1.72,441.94,37.61.36,96.45,1,158,1.76C827.13,444.71,916.56,445.74,960.32,446Z"/>
                <path data-aos="fade-down" data-aos-delay="600"d="M878.3,511.06l-26.45,21c-46.49-.39-110.07-1-172.16-1.59-76.19-.72-148.16-1.41-179.86-1.62l-10.52-.07V518.23c0-7.76.05-26.9.11-53.91.24-106.51.74-328.15-.11-464.09L510.49.09c.85,136,.34,357.74.1,464.28,0,18.54-.07,33.36-.09,43.34,35.44.27,100.7.89,169.39,1.55C754.69,510,831.73,510.7,878.3,511.06Z"/>
                <path data-aos="fade-down" data-aos-delay="300"d="M469.34,0c1.88,298.54,1.88,527.76,1.88,712,0,.44,0,.88,0,1.31C465.78,817.7,378.28,914.1,272,932.74A214.9,214.9,0,0,1,234.85,936c-41.16,0-81.28-12.09-117.74-35.71C70.52,870.11,31.21,822.34,0,758.08l40.94-32.45c53,115.51,133.42,172.6,222.28,157,83.69-14.67,152.6-89.88,157.16-171.39,0-184,0-412.93-1.88-710.95Z"/>
                <path data-aos="fade-down" data-aos-delay="300" d="M402.59.13c.62,211.91.43,387.84.27,543.06-.07,62.48-.13,120.73-.13,175.35,0,.32,0,.64,0,1-2.51,27.65-18.28,58.56-43.28,84.8-26.83,28.17-60.48,46.86-94.75,52.61a135.41,135.41,0,0,1-22.52,1.89c-48,0-119.3-25.49-177.71-151.79l17-13.49.08,0C128.21,797,192,847.63,261.14,836c29.79-5,59.24-21.46,82.93-46.33,21.3-22.35,35.28-49.09,37.48-71.64,0-54.48.06-112.58.13-174.87.16-155.2.35-331.11-.28-543Z"/>
            </svg>
        </a>
        <button class="gn-trigger" data-aos="fade-down" data-aos-delay="450" title="Hamburger Menu Trigger Button">
            <div class="burger-menu"></div>
        </button>
        <nav class="gn">
            <ul class="flex">
                <li data-aos="fade-down" data-aos-delay="550"><a href="https://jessicamlee.dev/#work" title="Work by Jessica">Work</a></li>
                <li data-aos="fade-down" data-aos-delay="600"><a href="about.php" title="Who is Jessica?">About</a></li>
                <li data-aos="fade-down" data-aos-delay="650"><a href="archive.php" title="Projects by Jessica">Archive</a></li>
                <li data-aos="fade-down" data-aos-delay="700" class="resume"><a href="https://docs.google.com/document/d/1JziKZSwMjRLoZD04V2UlWlJ-cW9i7XsUoCCYsifGE4g/edit?usp=sharing" title="Jessica's Résumé">R&eacute;sum&eacute;</a></li>
                <li class="flex header-icons">
                    <a href="https://www.linkedin.com/in/jessicamlee-/" target="_blank" title="Visit Jessica's LinkedIn">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 258.56 258.56" title="LinkedIn Icon"><defs></defs><path d="M129.28,0A129.28,129.28,0,1,0,258.56,129.28,129.28,129.28,0,0,0,129.28,0ZM107.73,172.37H86.19V107.73h21.54ZM97,98.13a11.95,11.95,0,1,1,11.85-11.94A11.89,11.89,0,0,1,97,98.13Zm86.18,74.24H161.62V141.55c0-20.27-21.57-18.55-21.57,0v30.82H118.5V107.73h21.55v11.78c9.39-17.41,43.09-18.71,43.09,16.67Z"/></svg>
                    </a>
                    <a href="https://github.com/jessicamlee" target="_blank" title="Visit Jessica's GitHub">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 258.95 253.48" title="GitHub Icon"><defs></defs><path d="M130.08,0A129.94,129.94,0,0,0,89,253.27c6.54,1.2,8.85-2.87,8.85-6.3v-22.1c-36.3,7.9-43.95-17.47-43.95-17.47a34.82,34.82,0,0,0-14.44-19.06c-11.73-8,1-8,1-8a27.39,27.39,0,0,1,19.86,13.4A27.74,27.74,0,0,0,98,204.66l.1,0a27.59,27.59,0,0,1,8-17.39C77.19,184,46.88,172.78,46.88,123.4A50.26,50.26,0,0,1,60.2,88.54a47.36,47.36,0,0,1,1.28-34.38S72.4,50.65,97.21,67.49a122.9,122.9,0,0,1,65.09,0C187.11,50.65,198,54.16,198,54.16a47.17,47.17,0,0,1,1.6,34.15,50.22,50.22,0,0,1,13.32,34.85c0,49.94-30.39,60.87-59.35,63.82a30.72,30.72,0,0,1,8.85,23.93v35.66c0,4.23,2.32,7.5,8.94,6.22A129.94,129.94,0,0,0,130.08,0Z"/></svg>
                    </a>
                </li>
                <li data-aos="fade-left" data-aos-delay="700" class="toggle">
                    <input type="checkbox" class="checkbox" id="checkbox">
                    <label for="checkbox" class="label">
                        <i class="fas fa-moon"></i>
                        <i class="fas fa-sun"></i>
                        <div class="ball"></div>
                    </label>
                </li>
            </ul>
        </nav>
    </div>
</header>